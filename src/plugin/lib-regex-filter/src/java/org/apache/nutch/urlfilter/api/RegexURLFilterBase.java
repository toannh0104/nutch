...
//			System.out.println("bachtv: " + line);
			if (line.length() == 0) {
				continue;
			}
		...
		// customize
		for (String customLine : Crawler.customRegexUrlFilter) {
			if (customLine.length() == 0) {
				continue;
			}
			char first = customLine.charAt(0);
			boolean sign = false;
			switch (first) {
			case '+':
				sign = true;
			...
			case '#': // skip blank & comment lines
				continue;
			default:
				throw new IOException("Invalid custom first character: " + customLine);
			}

			String regex = customLine.substring(1);
			if (LOG.isTraceEnabled()) {
				LOG.info("---> Adding custom rule [" + regex + "]");
			}
			RegexRule rule = createRule(sign, regex);
			rules.add(rule);
		}
